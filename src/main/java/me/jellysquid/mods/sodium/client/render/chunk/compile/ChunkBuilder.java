...
public class ChunkBuilder<T extends ChunkGraphicsState> {
    ...
    private static int getThreadCount() {
        int requested = Vitrine.options().performance.chunkBuilderThreads;
        ...
        public void run() {
            ...
            while (this.running.get()) {
                ...
                try {
                    ...
                    Vitrine.logger().error("Chunk build failed", e);
                    continue;
                } finally {
        ...
        private WrappedTask<T> getNextJob() {
            ...
            if (job == null) {
                synchronized (ChunkBuilder.this.jobNotifier) {
                    // Re-check under the lock: a producer may have enqueued and notified between the poll
                    // above and acquiring the monitor, which would otherwise be a lost wakeup.
